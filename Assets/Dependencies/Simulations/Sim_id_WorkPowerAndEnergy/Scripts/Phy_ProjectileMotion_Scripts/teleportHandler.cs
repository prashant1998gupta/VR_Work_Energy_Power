...
namespace Inspirit.Simulations.ProjectileMotion
{
    public class teleportHandler : MonoBehaviour
    {
        ...
        void raycastUpdate()
        {
            ...
            {
                if (hits[i].collider.gameObject.name == "FloorQuad")
                {
                    ...
                    lineRendRef.SetPosition(1, hits[i].point);

                    lineRendRef.startWidth = 0.00126f * 4;
                    lineRendRef.endWidth = 0.00126f * 4;

                    telportPointIndicatorRef.transform.position = hits[i].point;
                    telportPointIndicatorRef.SetActive(true);

                    lastRaycastHitPoint = hits[i].point;
                }
                else
                {
                    lineRendRef.enabled = false;
